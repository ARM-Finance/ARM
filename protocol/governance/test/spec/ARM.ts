...
describe('ARM', () => {
    ...
    context('transfer', async () => {
        ...
        it('does not allow a transfer to the zero address', async () => {
            const amount = 100
            await expect(armToken.transfer(ZERO_ADDRESS, amount)).to.revertedWith("ARM::_transferTokens: cannot transfer to the zero address")
        })
    })
    ...
    context('transferFrom', async () => {
        ...
        it('cannot transfer in excess of the spender allowance', async () => {
            ...
            await expect(armToken.transferFrom(alice.address, bob.address, balance)).to.revertedWith("revert ARM::transferFrom: transfer amount exceeds allowance")
        })
    })
    ...
    context('transferWithAuthorization', async () => {
        ...
        it('does not allow a transfer before auth valid', async () => {
            ...
            await expect(armToken.transferWithAuthorization(
                ...
                ethers.utils.hexlify(r),
                ethers.utils.hexlify(s))
            ).to.revertedWith("revert ARM::transferWithAuth: auth not yet valid")
        })
        ...
        it('does not allow a transfer after auth expiration', async () => {
            ...
            await expect(armToken.transferWithAuthorization(
                ...
                ethers.utils.hexlify(r),
                ethers.utils.hexlify(s))
            ).to.revertedWith("revert ARM::transferWithAuth: auth expired");
        })
        ...
        it('does not allow a reuse of nonce', async () => {
            ...
            await expect(armToken.transferWithAuthorization(deployer.address, bob.address, value, validAfter, validBefore, nonce, sig.v, ethers.utils.hexlify(sig.r), ethers.utils.hexlify(sig.s))).to.revertedWith("revert ARM::transferWithAuth: auth already used")
        })
    })
    ...
    context('receiveWithAuthorization', async () => {
        it('allows a valid receive with auth', async () => {
            ...
            const balanceBefore = await armToken.balanceOf(alice.address)
            await armToken.connect(alice).receiveWithAuthorization(deployer.address, alice.address, value, validAfter, validBefore, nonce, v, ethers.utils.hexlify(r), ethers.utils.hexlify(s))
            expect(await armToken.balanceOf(alice.address)).to.eq(balanceBefore.add(value))
        })
        ...
        it('does not allow a user to receive a transfer intended for another user', async () => {
            ...
            await expect(armToken.connect(bob).receiveWithAuthorization(deployer.address, alice.address, value, validAfter, validBefore, nonce, v, ethers.utils.hexlify(r), ethers.utils.hexlify(s))).to.revertedWith("revert ARM::receiveWithAuth: caller must be the payee")
        })
        ...
        it('does not allow a receive before auth valid', async () => {
            ...
            await expect(armToken.connect(alice).receiveWithAuthorization(deployer.address, alice.address, value, validAfter, validBefore, nonce, v, ethers.utils.hexlify(r), ethers.utils.hexlify(s))).to.revertedWith("revert ARM::receiveWithAuth: auth not yet valid")
        })
        ...
        it('does not allow a receive after auth expiration', async () => {
            ...
            await expect(armToken.connect(alice).receiveWithAuthorization(deployer.address, alice.address, value, validAfter, validBefore, nonce, v, ethers.utils.hexlify(r), ethers.utils.hexlify(s))).to.revertedWith("revert ARM::receiveWithAuth: auth expired")
        })
        ...
        it('does not allow a reuse of nonce', async () => {
            ...
            await expect(armToken.connect(bob).receiveWithAuthorization(deployer.address, bob.address, value, validAfter, validBefore, nonce, sig.v, ethers.utils.hexlify(sig.r), ethers.utils.hexlify(sig.s))).to.revertedWith("revert ARM::receiveWithAuth: auth already used")
        })
    })
    ...
    context('permit', async () => {
        ...
        it('does not allow a permit after deadline', async () => {
            ...
            await expect(armToken.permit(deployer.address, alice.address, value, deadline, v, ethers.utils.hexlify(r), ethers.utils.hexlify(s))).to.revertedWith("revert ARM::permit: signature expired")
        })
    })
    ...
    context("mint", async () => {
        ...
        it('only supply manager can mint', async () => {
            await expect(armToken.connect(alice).mint(bob.address, 1)).to.revertedWith("revert ARM::mint: only the supplyManager can mint")
        })

        it('cannot mint to the zero address', async () => {
            await expect(armToken.mint(ZERO_ADDRESS, 1)).to.revertedWith("revert ARM::mint: cannot transfer to the zero address")
        })
        ...
        it('cannot mint in excess of the mint cap', async () => {
            ...
            await expect(armToken.mint(alice.address, maxAmount.add(1))).to.revertedWith("revert ARM::mint: exceeded mint cap")
        })

        it('cannot mint before supply change allowed', async () => {
            await expect(armToken.mint(alice.address, 1)).to.revertedWith("revert ARM::mint: minting not allowed yet")
        })
    })
    ...
    context("burn", async () => {
        ...
        it('only supply manager can burn', async () => {
            await expect(armToken.connect(alice).burn(deployer.address, 1)).to.revertedWith("revert ARM::burn: only the supplyManager can burn")
        })

        it('cannot burn from the zero address', async () => {
            await expect(armToken.burn(ZERO_ADDRESS, 1)).to.revertedWith("revert ARM::burn: cannot transfer from the zero address")
        })

        it('cannot burn before supply change allowed', async () => {
            await expect(armToken.burn(deployer.address, 1)).to.revertedWith("revert ARM::burn: burning not allowed yet")
        })
        ...
        it('cannot burn in excess of the spender balance', async () => {
            ...
            await expect(armToken.burn(alice.address, balance.add(1))).to.revertedWith("revert ARM::burn: burn amount exceeds allowance")
        })
        ...
        it('cannot burn in excess of the spender allowance', async () => {
            ...
            await armToken.transfer(alice.address, 100)
            const balance = await armToken.balanceOf(alice.address)
            await expect(armToken.burn(alice.address, balance)).to.revertedWith("revert ARM::burn: burn amount exceeds allowance")
        })
    })
    ...
    context("setSupplyManager", async () => {
        ...
        it('only supply manager can set a new supply manager', async () => {
            await expect(armToken.connect(alice).setSupplyManager(bob.address)).to.revertedWith("revert ARM::setSupplyManager: only SM can change SM")
        })
    })
    ...
    context("setMetadataManager", async () => {
        ...
        it('only metadata manager can set a new metadata manager', async () => {
            await expect(armToken.connect(alice).setMetadataManager(bob.address)).to.revertedWith("revert ARM::setMetadataManager: only MM can change MM")
        })
    })
    ...
    context("setMintCap", async () => {
        ...
        it('only supply manager can set a new mint cap', async () => {
            await expect(armToken.connect(alice).setMintCap(0)).to.revertedWith("revert ARM::setMintCap: only SM can change mint cap")
        })
    })
    ...
    context("setSupplyChangeWaitingPeriod", async () => {
        ...
        it('only supply manager can set a new supply change waiting period', async () => {
            const waitingPeriodMinimum = await armToken.supplyChangeWaitingPeriodMinimum()
            await expect(armToken.connect(alice).setSupplyChangeWaitingPeriod(waitingPeriodMinimum)).to.revertedWith("revert ARM::setSupplyChangeWaitingPeriod: only SM can change waiting period")
        })

        it('waiting period must be > minimum', async () => {
            await expect(armToken.setSupplyChangeWaitingPeriod(0)).to.revertedWith("revert ARM::setSupplyChangeWaitingPeriod: waiting period must be > minimum")
        })
    })
    ...
    context("updateTokenMetadata", async () => {
        ...
        it('only metadata manager can update token metadata', async () => {
            await expect(armToken.connect(alice).updateTokenMetadata("New Token", "NEW")).to.revertedWith("revert ARM::updateTokenMeta: only MM can update token metadata")
        })
    })
})