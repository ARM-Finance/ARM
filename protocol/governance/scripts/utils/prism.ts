import { getFunctionSigs } from "./contracts";

export function noSelectorClashes(prism, implementation) {
    ...
    for (const impSig of impSigs) {
        for (const prismSig of prismSigs) {
            if (impSig.sig === prismSig.sig) {
                noClashes = false;
                console.log(
                    "Function: " + impSig.name +
                    " in implementation contract clashes with " + prismSig.name +
                    " in prism contract (signature: " + impSig.sig + ")"
                );
