...
contract VotingPower is PrismProxyImplementation, ReentrancyGuardUpgradeSafe {
	 ...
	 */
	function initialize(address _armToken, address _vestingContract) public initializer {
		__ReentrancyGuard_init_unchained();
		AppStorage storage app = VotingPowerStorage.appStorage();
		app.armToken = IARM(_armToken);
	...
	function stakeWithPermit(
		...
		AppStorage storage app = VotingPowerStorage.appStorage();
		require(app.armToken.balanceOf(msg.sender) >= amount, "VP::stakeWithPermit: not enough tokens");

		app.armToken.permit(msg.sender, address(this), amount, deadline, v, r, s);

		_stake(msg.sender, address(app.armToken), amount, amount);
	}
	...
	function stake(uint256 amount) external nonReentrant {
		...
		require(app.armToken.balanceOf(msg.sender) >= amount, "VP::stake: not enough tokens");
		require(
			app.armToken.allowance(msg.sender, address(this)) >= amount,
