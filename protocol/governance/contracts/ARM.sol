...
contract ARM {
    ...
    constructor(address _metadataManager, address _supplyManager, uint256 _firstSupplyChangeAllowed) {
        require(_firstSupplyChangeAllowed >= block.timestamp, "ARM::constructor: minting can only begin after deployment");
    ...
    function burn(address src, uint256 amount) external returns (bool) {
        ...
        if (spender != src && spenderAllowance != uint256(-1)) {
            uint256 newAllowance = spenderAllowance.sub(
                amount,
                "ARM::burn: burn amount exceeds allowance"
            );
    ...
    function setSupplyChangeWaitingPeriod(uint32 period) external returns (bool) {
        ...
        require(period >= supplyChangeWaitingPeriodMinimum, "ARM::setSupplyChangeWaitingPeriod: waiting period must be > minimum");
    ...
    function transferFrom(address src, address dst, uint256 amount) external returns (bool) {
        ...
        if (spender != src && spenderAllowance != uint256(-1)) {
            uint256 newAllowance = spenderAllowance.sub(
                amount,
                "ARM::transferFrom: transfer amount exceeds allowance"
            );
            allowances[src][spender] = newAllowance;
    ...
    function _decreaseAllowance(
        ...
        _approve(
            ...
            allowances[owner][spender].sub(
                subtractedValue,
                "ARM::_decreaseAllowance: decreased allowance below zero"
            )
        );
    }
    ...
    function _transferTokens(address from, address to, uint256 value) internal {
        ...
        balances[from] = balances[from].sub(
            value,
            "ARM::_transferTokens: transfer exceeds from balance"
        );
    ...
    function _burn(address from, uint256 value) internal {
        balances[from] = balances[from].sub(
            value,
            "ARM::_burn: burn amount exceeds from balance"
        );
        totalSupply = totalSupply.sub(
            value,
            "ARM::_burn: burn amount exceeds total supply"
        );
