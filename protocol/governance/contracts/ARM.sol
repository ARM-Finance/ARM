...
/**
 * @title ARM
 * @dev The governance token for ARM Finance
...
contract ARM {
	...
	string public name = "ARM Governance Token";
	...
	uint256 public totalSupply = 10000e18; // 10 thousands
	...
	constructor(
		...
		require(
			...
			"ARM::constructor: minting can only begin after deployment"
		);
	...
	function setSupplyManager(address newSupplyManager) external returns (bool) {
		require(msg.sender == supplyManager, "ARM::setSupplyManager: only SM can change SM");
	...
	function setMetadataManager(address newMetadataManager) external returns (bool) {
		require(msg.sender == metadataManager, "ARM::setMetadataManager: only MM can change MM");
	...
	function mint(address dst, uint256 amount) external returns (bool) {
		require(msg.sender == supplyManager, "ARM::mint: only the supplyManager can mint");
		require(dst != address(0), "ARM::mint: cannot transfer to the zero address");
		require(amount <= totalSupply.mul(mintCap).div(1000000), "ARM::mint: exceeded mint cap");
		require(block.timestamp >= supplyChangeAllowedAfter, "ARM::mint: minting not allowed yet");
	...
	function burn(address src, uint256 amount) external returns (bool) {
		...
		require(spender == supplyManager, "ARM::burn: only the supplyManager can burn");
		require(src != address(0), "ARM::burn: cannot transfer from the zero address");
		require(block.timestamp >= supplyChangeAllowedAfter, "ARM::burn: burning not allowed yet");

		uint256 spenderAllowance = allowances[src][spender];
		// check allowance and reduce by amount
		if (spender != src && spenderAllowance != uint256(-1)) {
			uint256 newAllowance = spenderAllowance.sub(amount, "ARM::burn: burn amount exceeds allowance");
	...
	function setMintCap(uint16 newCap) external returns (bool) {
		require(msg.sender == supplyManager, "ARM::setMintCap: only SM can change mint cap");
	...
	function setSupplyChangeWaitingPeriod(uint32 period) external returns (bool) {
		require(msg.sender == supplyManager, "ARM::setSupplyChangeWaitingPeriod: only SM can change waiting period");
		require(
			period >= supplyChangeWaitingPeriodMinimum,
			"ARM::setSupplyChangeWaitingPeriod: waiting period must be > minimum"
		);
	...
	function updateTokenMetadata(string memory tokenName, string memory tokenSymbol) external returns (bool) {
		require(msg.sender == metadataManager, "ARM::updateTokenMeta: only MM can update token metadata");
		name = tokenName;
		symbol = tokenSymbol;
	...
	function permit(
		...
		require(deadline >= block.timestamp, "ARM::permit: signature expired");
	...
	function transferFrom(
		...
		if (spender != src && spenderAllowance != uint256(-1)) {
			uint256 newAllowance = spenderAllowance.sub(amount, "ARM::transferFrom: transfer amount exceeds allowance");
			allowances[src][spender] = newAllowance;
	...
	function transferWithAuthorization(
		...
		require(block.timestamp > validAfter, "ARM::transferWithAuth: auth not yet valid");
		require(block.timestamp < validBefore, "ARM::transferWithAuth: auth expired");
		require(!authorizationState[from][nonce], "ARM::transferWithAuth: auth already used");
	...
	function receiveWithAuthorization(
		...
		require(to == msg.sender, "ARM::receiveWithAuth: caller must be the payee");
		require(block.timestamp > validAfter, "ARM::receiveWithAuth: auth not yet valid");
		require(block.timestamp < validBefore, "ARM::receiveWithAuth: auth expired");
		require(!authorizationState[from][nonce], "ARM::receiveWithAuth: auth already used");
	...
	function _validateSignedData(
		...
		require(recoveredAddress != address(0) && recoveredAddress == signer, "ARM::validateSig: invalid signature");
	}
	...
	function _approve(
		...
		require(owner != address(0), "ARM::_approve: approve from the zero address");
		require(spender != address(0), "ARM::_approve: approve to the zero address");
	...
	function _decreaseAllowance(
		...
		_approve(
			owner,
			spender,
			allowances[owner][spender].sub(subtractedValue, "ARM::_decreaseAllowance: decreased allowance below zero")
		);
	}
	...
	function _transferTokens(
		...
		require(to != address(0), "ARM::_transferTokens: cannot transfer to the zero address");

		balances[from] = balances[from].sub(value, "ARM::_transferTokens: transfer exceeds from balance");
	...
	function _burn(address from, uint256 value) internal {
		balances[from] = balances[from].sub(value, "ARM::_burn: burn amount exceeds from balance");
		totalSupply = totalSupply.sub(value, "ARM::_burn: burn amount exceeds total supply");
