...
contract Vault {
	...
	function lockTokens(
		...
		require(
			IUniswapV2ERC20(token).transferFrom(locker, address(this), amount),
			"Vault::lockTokens: transfer failed"
		);
	...
	function lockTokensWithPermit(
		...
		require(
			IUniswapV2ERC20(token).transferFrom(locker, address(this), amount),
			"Vault::lockTokensWithPermit: transfer failed"
		);
	...
	function claimAllUnlockedTokens(uint256 lockId) external {
		...
		require(
			IUniswapV2ERC20(lock.token).transfer(lock.receiver, unlockedAmount),
			"Vault::claimAllUnlockedTokens: transfer failed"
		);
	...
	function claimUnlockedTokens(uint256 lockId, uint256 amount) external {
		...
		require(
			IUniswapV2ERC20(lock.token).transfer(lock.receiver, amount),
			"Vault::claimUnlockedTokens: transfer failed"
		);
